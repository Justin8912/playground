...
export class HCPVaultService {
    ...
    constructor(vaultToken: string) {
        this.initializeVaultClient(vaultToken);
    }

    public initializeVaultClient = (vaultToken: string) => {
    ...
    public setUserSpotifyCredentials = async (user: string, accessToken: AccessToken): Promise<void> => {
        logger.info(`Setting spotify user credentials for ${user}`);
        await this.setParameter(`spotify/${user}`, accessToken);
    }
    ...
    public setUserGoogleCredentials = async (user: string, accessToken: GoogleCredentials): Promise<void> => {
        logger.info(`Setting google user credentials for ${user}`);
